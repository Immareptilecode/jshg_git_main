	...
	$('#benef').modal({
    blurring: true
  }).modal('attach events', '#launch_benef', 'show');

	$('#payshares').modal({
	    blurring: true
	  }).modal('attach events', '#launchpayshares', 'show');

	$('#paysharecapital').modal({
	    blurring: true
	  }).modal('attach events', '#launchpaysharecapital', 'show');
...
var get_charges_individual=function(url){
...
var get_fines_individual=function(url){
...
var get_fines_individual_after_payfine=function(url){
...
var get_charges_individual_after_paycharge=function(url){
...
var get_fines_individual_after_addfine=function(url){
...
var get_charges_individual_after_addcharge=function(url){
	...
	$.get(
		...
		function(data){
			var total=0
			the_added_charge=data[data.length-1]
...
var charge_acc=function(url, member_id){
	var csrftoken = jQuery("[name=csrfmiddlewaretoken]").val();
		$.ajax({
			url:url,
			type: "POST",
			data:{csrfmiddlewaretoken: csrftoken, Amount_due:$('#charge_amount').val(), Date:$('#charge_date').val(), member:member_id, Particulars:$('#charge_parts').val()},


			success:function(message){
				$("#table_body_charges").prepend('<tr><td>'+ $('#charge_parts').val() +'</td> <td>'+ $('#charge_amount').val() + '</td> <td> '+ $('#charge_date').val() +'</td> </tr>')
				get_charges_individual_after_addcharge(url)
...
var fine_acc=function(url, member_id){
	var csrftoken = jQuery("[name=csrfmiddlewaretoken]").val();
		$.ajax({
			url:url,
			type: "POST",
			data:{csrfmiddlewaretoken: csrftoken, Amount_due:$('#fine_amount').val(), Date:$('#fine_date').val(), member:member_id, Particulars:$('#fine_parts').val()},


			success:function(message){
				$("#table_body_fines").prepend('<tr><td>'+$('#fine_parts').val()+'</td> <td>'+ $('#fine_amount').val() + '</td> <td> '+ $('#fine_date').val()+'</td> </tr>')
				get_fines_individual_after_addfine(url)
...
var pay_charges=function(url, get_charges_url){
 ...
 var csrftoken = jQuery("[name=csrfmiddlewaretoken]").val();
		$.ajax({
			...
			success:function(message){
				get_charges_individual_after_paycharge(get_charges_url)
 ...
 var csrftoken = jQuery("[name=csrfmiddlewaretoken]").val();
		$.ajax({
			...
			success:function(message){
				...
				get_fines_individual_after_payfine(get_fines_url)
				if (message === 'insufficient_funds'){
...
var Indi_debt_ops = Indi_debt_ops || (function(){
    var _args = {}; // private
